  ...
  return <section>
    ...
    {Object.keys(leaderboard)
      ...
      .map(key => {
        ...
        return <ul style={{ listStyle: 'none', padding: 0, textAlign: 'center' }} key={key}>
          <h3 className='leaderboard-group-heading' onClick={e => chooseGame(rows, cols, difficulty)}>
            {difficulties[difficulty]}: {rows} x {cols}
          </h3>
          {leaderboard[key]
            .map((record, i) => <li className='record-item' key={i}>
              <span className='record-time'>{formatTimer(record.time)}</span>
              {' '}
